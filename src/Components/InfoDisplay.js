...
export default class InfoDisplay extends Component {
  _renderWeather(props) {
    var weatherSummary = '';
    if (props) {
      weatherSummary = `Weather outlook: ${props.summary} Temperature: High - ${
        props.highTemp
      } Low - ${props.lowTemp} * Wind: ${props.windSpeed}`;
    }
  ...
  render() {
    ...
    return (
      <div style={this.props.collapse ? collapseStlye : expandStlye}>
        ...
        <h6>{this._renderWeather(this.props.event.weather)}</h6>
        <div
        ...
        <div style={container}>
          <span id="collapse" style={click}>
            Show {this.props.collapse ? 'More' : 'Less'}
          </span>
        </div>
      </div>
    );
